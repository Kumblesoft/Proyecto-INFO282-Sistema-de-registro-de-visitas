...
const TextEntry = ({ optionalFeatures, onSelect, requiredFieldRef }) => {
  ...
  const handleChange = (text) => {
    ...
    if (isValid) {
      ...
      setIsRequiredAlert(false) // Reset the alert style when valid input is given
    }
  }
  ...
  return (
    <KeyboardAvoidingView
      ...
      <Input
        style={isRequiredAlert ? styles.alertRequired : styles.input}
        value={inputValue}
...
const styles = StyleSheet.create({
  ...
  alertRequired: {
    borderWidth: 1,
    borderColor: '#ff0000', //Estilo de campos obligatorios
    padding: 10,
    borderRadius: 5,
    backgroundColor: '#fff',
    fontSize: 15,
